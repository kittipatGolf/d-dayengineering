...
export function ProductCard({
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <h3 className="mt-4 text-lg sm:text-2xl font-bold leading-tight text-slate-700">
        {title}
      </h3>
      <p className="mt-2 text-sm sm:text-base text-slate-600">{description}</p>
      {features && features.length > 0 ? (
        <div className="mt-4 rounded-xl border border-slate-300 bg-slate-50 p-3 sm:p-4">
      ...
      <p className="mt-3 text-sm sm:text-base font-extrabold text-red-600">{price}</p>
    </article>
  );
}
